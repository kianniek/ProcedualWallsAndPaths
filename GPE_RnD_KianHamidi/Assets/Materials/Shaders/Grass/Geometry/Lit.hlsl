...
float4 frag(Varyings input) : SV_Target
{
    ...
    float3 maskColor = tex2D(_MaskTexture, input.uv).rgb;
    
    // Threshold to determine if the mask color is "black enough" to discard.
    // Adjust the 0.01 value as needed to make it more or less sensitive.
    if(length(maskColor) > 0.01)
    {
        discard; // Discards the fragment, making it transparent/not rendered
    }
